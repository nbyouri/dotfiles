...
# path
export PATH=/usr/pkg/cross/bin:/usr/pkg/xv6/bin:/usr/pkg/lib/cw:/Volumes/pkgsrc/pkg/bin:/Volumes/pkgsrc/pkg/sbin:/usr/local/homebrew/bin:~/bin:$PATH:/usr/local/sbin:/usr/local/bin:/usr/local/sbin:/usr/libexec:/opt/local/bin:/opt/local/sbin:/usr/local/mysql/bin:/usr/pkg/bin:/usr/pkg/sbin
export MANPATH="/usr/pkg/man:$MANPATH"

# Go env
export GOROOT=/usr/pkg/go
...
function git_prompt_info() {
        ...
        echo "%B%F{black}─%f(%{$fg[yellow]%}${ref#refs/heads/}%{$reset_color%})"
}

PROMPT="
${git_branch}%B%F{black}──── %f"
...
alias l='ls -lah'
alias up=' rsync -avhz --progress /Volumes/Backup/pkgsrc_2013Q4/packages/ root@saveosx.org:/usr/local/www/saveosx/packages/Darwin/2013Q4/x86_64/'
alias pup='sudo pkgin up'
alias pin='sudo pkgin -y in'
alias pav='pkgin av'
alias pls='pkgin ls'
alias sauce='source ~/.zshrc'
alias spc='spc -c /usr/pkg/share/supercat/spcrc-c'
...
upb () {
    ...
    rsync -ahz _site/* root@saveosx.org:/var/www/saveosx/blog/; 
    git add -A;
    git commit -m 'update';
    git push --quiet;
    cd -;
...
add () {
    ...
    scp $1 ssx:/usr/local/www/saveosx/packages/Darwin/2013Q3/x86_64/All/ 2>/dev/null;
    echo "Updating the package summary...";
    ssh ssx 'cd /usr/local/www/saveosx/packages/Darwin/2013Q3/x86_64/All/;
    rm pkg_summary.gz;
